...
#gonna make like a alert that notify bug is stunned
...
func stun_bug():
	is_stunned = true
	print("Bug is stunned for ", stun_duration, " seconds.")
	# Wait for the stun duration, then un-stun the bug
	await get_tree().create_timer(stun_duration).timeout
	is_stunned = false
	print("Bug is no longer stunned.")
